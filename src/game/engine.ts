...
const TOWER_BONDS: Record<TowerKind, TowerBondDefinition> = {
  ...
  pike: { id: "brand", label: "Brand", partner: "cinder" },
  cinder: { id: "brand", label: "Brand", partner: "pike" },
};
...
export class EmberEngine {
  ...
  kindredRate(tower: Tower) {
    for (const other of this.towers) {
      if (other.id === tower.id) continue;
      if (other.kind !== tower.kind) continue;
      if (Math.abs(other.c - tower.c) + Math.abs(other.r - tower.r) === 1) return KINDRED_RATE;
    }
    return 1;
  }
  ...
  renderText() {
    ...
    return JSON.stringify({
      coordinateSystem:
        "origin top-left; x increases right; y increases down; board is 13 columns by 9 rows",
      phase: hud.phase,
      map: hud.mapName,
      field: hud.field,
      wave: {
  ...
  objectiveProgress() {
    ...
    if (rule.id === "lantern-aura") {
      ...
      current = Math.min(
        rule.target,
        this.towers.filter((tower) => this.besideWater(tower)).length,
      );
    } else if (rule.id === "ash-draw") {
      current = this.towers.some(
        (tower) => (tower.kind === "cinder" || tower.kind === "mortar") && this.besidePath(tower),
      )
        ? 1
        : 0;
    } else if (rule.id === "wicker-draft") {
      current = this.towers.some(
        (tower) => (tower.kind === "frost" || tower.kind === "bramble") && this.besidePath(tower),
      )
        ? 1
        : 0;
    }
  ...
  watchOrderSnapshot(): WatchOrderSnap | null {
    ...
    const previewIndex = this.endless
      ? Math.max(0, index)
      : Math.min(index, Math.max(0, this.map.waves.length - 1));
    const plan = this.wavePlan(previewIndex);
    const order = this.phase === "wave" && this.waveOrder ? this.waveOrder : watchOrderFor(plan);
    if (!order) return null;
    const current = order.targetKind
      ? Math.min(order.target, this.waveKillCounts[order.targetKind] ?? 0)
      : 0;
    return {
      ...order,
      current,
  ...
  inLanternAura(tower: Tower) {
    return this.props.some(
      (prop) =>
        prop.kind === "lamp" && Math.abs(prop.c - tower.c) + Math.abs(prop.r - tower.r) <= 2,
    );
  }
    ...
    return Math.min(
      ...
      this.towers.filter((tower) =>
        this.towers.some(
          (other) =>
            other.id !== tower.id &&
            Math.abs(other.c - tower.c) + Math.abs(other.r - tower.r) === 1,
        ),
      ).length,
    );
  }
  ...
  fieldDamageMultiplier(tower: Tower) {
    ...
    if (
      this.map.profile.rule.id === "glass-tide" &&
      tower.kind === "spark" &&
      this.besideWater(tower)
    )
      return 1.12;
    if (this.map.profile.rule.id === "ash-draw" && tower.kind === "cinder") return 1.14;
    return 1;
  }
  ...
  buildHud(): HudSnap {
    ...
    return {
      ...
      nextCosts: t
        ? {
            dmg:
              t.dmgLvl >= MAX_UPGRADE
                ? 0
                : this.upgradePrice(upgradeDamageCost(t.kind, t.dmgLvl + 1)),
            rate:
              t.rateLvl >= MAX_UPGRADE
                ? 0
                : this.upgradePrice(upgradeRateCost(t.kind, t.rateLvl + 1)),
            range:
              t.rangeLvl >= MAX_UPGRADE
                ? 0
                : this.upgradePrice(upgradeRangeCost(t.kind, t.rangeLvl + 1)),
          }
        : null,
      aim: this.aim,
      paused: this.paused,
      ...
      flareCd: this.flareCd,
      flareCost: FLARE_COST,
      ...
      marked: (() => {
        ...
        return {
          name: CREEPS[marked.kind].name,
          kind: marked.kind,
          hp: Math.max(0, Math.ceil(marked.hp)),
          maxHp: marked.maxHp,
      ...
      relicNames: SHOP.filter((item) => this.relics.has(item.id)).map((item) => ({
        id: item.id,
        name: item.name,
      })),
      rite: this.rite,
      ...
      ability: t
        ? {
            ...
            ready:
              t.abilityCd <= 0 && this.playing() && !t.volt && !t.siege && !t.storm && !t.brace,
          }
        : null,
      emberlitOptions:
        t && this.formOf(t) >= 4 && !t.empowered
          ? { a: EMBERLIT[t.kind].a, b: EMBERLIT[t.kind].b }
          : null,
      marks: this.marks,
      ...
      omen: (() => {
        ...
        return omen
          ? { id: omen.id, name: omen.name, detail: omen.detail, color: omen.color }
          : null;
      })(),
      bestEndless: this.bestEndless,
      endless: this.endless,
  ...
  refundFor(t: Tower) {
    const rate =
      t.dmgLvl === 1 && t.rateLvl === 1 && t.rangeLvl === 1 && !t.empowered ? 0.7 : REFUND_RATE;
  ...
  scoutFlare() {
    ...
    if (this.flareCd > 0 || this.gold < FLARE_COST) {
      sfx.deny();
      return;
    }
    this.gold -= FLARE_COST;
    this.flareCd = FLARE_CD;
    this.flareT = FLARE_DURATION;
    let marked = 0;
    for (const creep of this.creeps) {
      if (!creep.alive) continue;
      creep.markedT = FLARE_DURATION;
      marked += 1;
      this.ring(creep.x, creep.y, "#f1d58a");
    }
    this.burst(COLS / 2, ROWS / 2, "#f1d58a", 14, "spark");
    this.float(COLS / 2, 0.52, marked > 0 ? `Marked ${marked}` : "Road marked", "#f1d58a");
    this.banner = {
      text: marked > 0 ? `Scout flare · ${marked} marked` : "Scout flare · watching the road",
      life: 1.4,
      max: 1.4,
    };
    sfx.flare();
  ...
  scoutMark() {
    ...
    for (const creep of this.creeps) {
      if (!creep.alive) continue;
      if (!best || creep.hp > best.hp || (creep.hp === best.hp && creep.progress > best.progress))
        best = creep;
    }
    if (!best) {
      sfx.deny();
  ...
  dismissBrief() {
    ...
    if (fresh.length > 0) {
      this.banner = {
        text: `${fresh.map((kind) => TOWERS[kind].name).join(" · ")} unsealed`,
        life: 2.4,
        max: 2.4,
      };
    }
  ...
  towerUnlocked(kind: TowerKind) {
    return this.unlocked >= TOWER_UNLOCK[kind] || this.mapIndex >= TOWER_UNLOCK[kind];
  }

  canStrike(tower: Tower, creep: Creep) {
  ...
  chooseKind(kind: TowerKind | null) {
    if (kind && !this.towerUnlocked(kind)) {
      this.rejectAction(TOWER_UNLOCK_HINT[kind] || "Sealed");
      return;
    }
  ...
  useAbility() {
    ...
    if (t.kind === "bow") {
      ...
      this.ring(
        cx,
        cy,
        t.kind === "frost" ? "#6aa8b4" : t.kind === "bramble" ? "#4a6a32" : "#d4a054",
      );
  ...
  creepNear(c: number, r: number): Creep | null {
    const cx = c + 0.5;
    const cy = r + 0.5;
    let best: Creep | null = null;
    let bestD = 0.72;
    for (const creep of this.creeps) {
      if (!creep.alive) continue;
      const d = Math.hypot(creep.x - cx, creep.y - cy);
      if (d < bestD) {
        bestD = d;
        best = creep;
      }
  ...
  markCreep(creep: Creep) {
    ...
    this.float(creep.x, creep.y - 0.45, action, "#e07838");
    this.banner = { text: `${action} ${CREEPS[creep.kind].name}`, life: 1.1, max: 1.1 };
    if (marked) this.ring(creep.x, creep.y, "#e07838");
    sfx.place();
    this.notify();
  }
  ...
  spawn(kind: CreepKind) {
    ...
    this.creeps.push({
      ...
      healT: 1.2,
      rootT: 0,
      markedT: this.flareT > 0 ? this.flareT : 0,
      dodge: kind === "knave",
      howled: false,
      hasteT: 0,
      elite,
    ...
    if (def.effect === "summon") {
      for (let i = 0; i < 3; i++) {
        this.spawnAt(
          i % 2 === 0 ? "hound" : "grub",
          creep.x + (i - 1) * 0.7,
          creep.y + (i % 2 === 0 ? 0.5 : -0.5),
        );
      }
    } else if (def.effect === "burn") {
      for (let i = 0; i < 5; i++) {
        const node =
          this.path[Math.min(this.path.length - 1, Math.max(0, Math.floor(creep.progress) + i))];
  ...
  damageCreep(creep: Creep, amount: number, slow: number, ignoreArmor = false, quiet = false) {
    ...
    if (creep.hp <= 0 && creep.alive) {
      ...
      const deathColor =
        creep.kind === "lord" ? "#e07838" : creep.kind === "shell" ? "#7a8470" : "#5a7a48";
      this.poof(
        creep.x,
        creep.y,
        deathColor,
        creep.kind === "lord" ? 2.4 : creep.kind === "shell" ? 1.4 : 1,
      );
  ...
  spawnSplinter(from: Creep) {
    const stats = CREEPS.grub;
    const hp = Math.max(8, Math.round(stats.hp * this.hpMult() * 0.7));
    this.creeps.push({
      id: this.nextId++,
      kind: "grub",
      x: from.x,
  ...
  fire(tower: Tower, target: Creep) {
    ...
    if (
      (tower.kind === "frost" || tower.kind === "ward") &&
      (target.kind === "hound" || target.kind === "ashfang")
    )
      dmg *= 1.22;
    ...
    if (tower.kind === "pike" && (target.kind === "shell" || target.kind === "knave")) dmg *= 1.22;
    if (tower.kind === "pike" && target.kind === "lord") dmg *= 1.14;
    if (tower.kind === "cinder" && target.kind === "moth") dmg *= 1.28;
    if (tower.kind === "cinder" && target.kind === "grub") dmg *= 1.12;
    if (tower.volley) {
    ...
    if (tower.kind === "ward") {
      ...
      const slow =
        def.slow +
        (form >= 3 ? 0.12 : 0) +
        (tower.emberlit === "b" ? 0.12 : 0) +
        (tower.empowered && tower.emberlit !== "b" ? 0.08 : 0);
      for (const c of this.creeps) {
        if (!c.alive) continue;
    ...
    const splash =
      tower.kind === "frost" && (form >= 3 || tower.empowered)
        ? form >= 4
          ? 0.95
          : 0.7
        : tower.kind === "cinder"
          ...
          : def.splash *
            (siege ? 1.6 : 1) *
            (tower.emberlit === "b" && tower.kind === "mortar" ? 1.25 : 1);
    if (def.beam) {
      this.beams.push({
        x1: sx,
        y1: sy,
      ...
      const hops = storm
        ? 6
        : (form >= 4 ? 2 : form >= 3 ? 1 : 0) + (tower.emberlit === "a" ? 1 : 0);
      if (hops > 0) {
    ...
    const spark =
      tower.kind === "frost"
        ? "#6aa8b4"
        : tower.kind === "mortar" || tower.kind === "cinder"
          ? "#e07838"
          : "#d4a054";
    if (tower.kind === "mortar" || tower.kind === "cinder") this.burst(sx, sy, spark, 2, "spark");
    if (tower.kind === "bow" || tower.kind === "pike") sfx.shootBow();
  ...
  pickTarget(tower: Tower, ignore: Set<number> = new Set()): Creep | null {
    ...
    if (marked && this.canStrike(tower, marked)) {
      ...
      const mdy = marked.y - cy;
      if (mdx * mdx + mdy * mdy <= r2) return marked;
    }
    let best: Creep | null = null;
    let score = tower.aim === "last" || tower.aim === "close" ? Infinity : -1;
    for (const creep of this.creeps) {
      if (!creep.alive || ignore.has(creep.id)) continue;
      if (!this.canStrike(tower, creep)) continue;
      const dx = creep.x - cx;
      const dy = creep.y - cy;
  ...
  impact(shot: Shot, x: number, y: number) {
    ...
    if (shot.splash > 0) {
      ...
      if (shot.kind === "mortar" || shot.kind === "cinder") {
        ...
        const fat = (shot.empowered ? 1.18 : 1) * (shot.kind === "cinder" ? 0.92 : 1);
        const ash = this.map.profile.rule.id === "ash-draw" ? 1.35 : 1;
        this.burns.push({
          x,
          y,
  ...
  step(dt: number) {
    ...
    for (const creep of this.creeps) {
      ...
      const pack =
        (creep.kind === "hound" || creep.kind === "ashfang") && packCount >= 3 ? 1.12 : 1;
      const draft =
    ...
    for (const burn of this.burns) {
      ...
      if (burn.tick >= 0.4) {
        burn.tick = 0;
        for (const creep of this.creeps) {
          if (!creep.alive || (CREEPS[creep.kind].flying && !CREEPS[creep.kind].low)) continue;
  ...
  finishWaveIfClear() {
    ...
    if (final) {
      ...
      if (this.mapIndex < MAPS.length - 1) {
        ...
        this.persist();
        if (!objective.complete)
          this.banner = { text: "Dawn — the line held", life: 2.2, max: 2.2 };
        sfx.win();
      }
      this.trauma = 0.25;
      if (orderHeld && order) {
  ...
  stepFx(dt: number) {
    ...
    if (!this.reducedMotion && !this.paused && this.moteAcc > 0.14 && this.particles.length < 140) {
      this.moteAcc = 0;
      const node =
        this.path[Math.floor(Math.random() * Math.max(1, this.path.length - 1))] ?? this.path[0];
