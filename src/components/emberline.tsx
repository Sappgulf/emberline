...
export function Emberline() {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "?" || e.key === "/") {
        ...
        helpRestoreRef.current =
          engine.phase === "title" ? titleHelpTriggerRef : headerHelpTriggerRef;
        engine.toggleHelp();
        return;
      }
      if (engine.help) {
  ...
  const stallHint =
    hud.wave < 1
      ...
      : hud.phase !== "ready"
        ? "Stall opens between waves"
        : "Open roadside stall";
  const mendHint =
    hud.lives >= hud.maxLives
      ? "The keep is already at full strength"
      : hud.gold < hud.mendCost
        ? `Mend costs ${hud.mendCost} gold`
        : `Mend the keep for ${hud.mendCost} gold`;
  ...
  const waveProgressLabel = hud.endless
    ...
    : hud.phase === "wave"
      ...
      : hud.wave > 0
        ? `Wave ${hud.wave} held`
        : "First watch";
  const waveProgressStatus =
    hud.phase === "wave"
      ? `${hud.remaining} left`
      : hud.wave > 0
        ? holdLabel(hud.lastResult?.hold)
        : "Ready";
  ...
  return (
    ...
    >
      ...
      <header className="watch-bar flex shrink-0 items-stretch" data-phase={hud.phase}>
        <div className="watch-title flex min-w-0 flex-1 flex-col justify-center px-4 py-2">
          ...
          <p className="watch-map-name font-display text-xl leading-none text-copper">
            ...
            {hud.endless && (
              <span className="ml-2 font-sans text-[10px] tracking-[0.18em] text-ember">
                LONG NIGHT
              </span>
            )}
            {hud.hard && (
              <span className="ml-2 font-sans text-[10px] tracking-[0.18em] text-ember">HARD</span>
            )}
          </p>
          {hud.phase !== "title" && (
            <div
              className="mt-1.5 flex gap-1"
              aria-label={`Map ${hud.mapIndex + 1} of ${hud.mapTotal}`}
            >
        ...
        {hud.phase !== "title" && (
          <div className="watch-vitals">
            ...
            <div className="watch-stat" data-stat="wave" title="Wave">
              ...
              <span className="u">
                {hud.phase === "wave" ? "left" : hud.endless ? "night" : "wave"}
              </span>
            </div>
          </div>
        )}
      ...
      <div className={`playfield-shell ${playing ? "playfield-shell-live" : ""}`}>
        {playing && <WatchDesk hud={hud} hint={placementMessageValue} tone={placementToneValue} />}
        <div
          ref={wrapRef}
        ...
        >
          ...
          {playing && hud.boss && (
            ...
            >
              <span className="boss-kicker">
                Boss · {hud.boss.phase > 0 ? "second wind" : "first phase"}
              </span>
              ...
              <span className="boss-track" aria-hidden="true">
                <span
                  style={{
                    width: `${Math.max(0, Math.round((hud.boss.hp / Math.max(1, hud.boss.maxHp)) * 100))}%`,
                  }}
                />
              </span>
            ...
            <div className="codex-tabs" role="tablist" aria-label="Codex sections">
              ...
              >
                ...
                <span className="codex-tab-count">
                  {hud.chronicle.filter((entry) => entry.unlocked).length}/{hud.chronicle.length}
                </span>
              </button>
            </div>
            {codexTab === "bestiary" ? (
              <div
                ...
                tabIndex={0}
                className="bestiary-grid w-full text-left"
              >
                {BESTIARY.map((b) => {
                    ...
                    <article key={b.kind} className="bestiary-card plaque">
                      ...
                      <div className="bestiary-copy">
                        <div className="bestiary-heading">
                          ...
                          <span className="bestiary-role">
                            {creep.flying ? "Air" : creep.armor > 0 ? "Armored" : "Ground"}
                          </span>
                        </div>
              ...
              <div
                id="codex-panel-chronicle"
                role="tabpanel"
                aria-labelledby="codex-tab-chronicle"
                tabIndex={0}
                className="chronicle-grid w-full text-left"
              >
                {hud.chronicle.map((entry) => (
                  <article
                    key={entry.id}
                    className="chronicle-card plaque"
                    data-locked={!entry.unlocked}
                  >
          ...
          >
            <div className="campaign-map-surface">
              <p className="max-w-md text-sm leading-relaxed text-dust">
                {Math.min(hud.mapTotal, hud.unlocked + 1)} of {hud.mapTotal} routes available.
                Select an open route to choose where the next watch begins.
              </p>
              ...
              <div className="campaign-select" aria-label="Campaign route selection">
                {hud.route.map((node, index) => {
                  ...
                  const pressure =
                    node.threatTier === "severe"
                      ? "Severe pressure"
                      : node.threatTier === "mixed"
                        ? "Mixed pressure"
                        : "Light pressure";
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="campaign-card-copy">
                        ...
                        <small>
                          {node.ruleLabel} · {node.objectiveTitle} · +{node.objectiveReward}g
                        </small>
          ...
          >
            <div className="orders-grid w-full text-left">
              {[
                ...
                [
                  "Bonds",
                  "Pair Bow + Frost, Mortar + Ward, Spark + Bramble, or Pike + Cinder for +8% power.",
                ],
          ...
          >
            <div className="hall-grid">
              <section className="hall-card plaque" aria-label="Watch marks">
                ...
                <p>
                  Marks are earned by holding roads, cutting bosses, and walking the Long Night.
                  Spend them on permanent watch perks.
                </p>
              </section>
              <section className="hall-card plaque" aria-label="Long Night">
                <span className="intel-kicker">Long Night</span>
                <strong>
                  {hud.unlocked >= hud.mapTotal ? `Best night ${hud.bestEndless}` : "Sealed"}
                </strong>
            ...
            <div className="hall-perks" aria-label="Watch perks">
              {hud.perkOptions.map((perk) => (
                <article
                  key={perk.id}
                  className="hall-perk plaque"
                  data-maxed={perk.tier >= perk.max}
                >
                  ...
                  <button
                    ...
                    aria-label={
                      perk.tier >= perk.max
                        ? `${perk.name} fully invested`
                        : `Invest in ${perk.name} for ${perk.cost} marks`
                    }
              ...
              <div className="keep-main">
                ...
                <label className="keep-hard">
                  <input
                    type="checkbox"
                    checked={hud.hard}
                    onChange={(e) => engine.setHard(e.target.checked)}
                ...
                <div className="menu-actions">
                  ...
                  <button
                    ref={codexTriggerRef}
                    type="button"
                    className="pressable stamp min-h-11 px-5 text-sm text-dust"
                    onClick={() => engine.toggleCodex()}
                  >
                    Bestiary
                  </button>
                  <button
                    ref={titleHelpTriggerRef}
                    type="button"
                    className="pressable stamp min-h-11 px-5 text-sm text-dust"
                    onClick={() => openHelp(titleHelpTriggerRef)}
                  >
                    Orders
                  </button>
                  <button
                    ref={hallTriggerRef}
                    type="button"
                    className="pressable stamp hall-trigger min-h-11 px-5 text-sm text-copper"
                    onClick={() => engine.toggleHall()}
                  >
                    Watch hall
                ...
                <p className="keep-whisper">
                  “The road bends. The watch holds.” — Sera Venn, watch-captain
                </p>
        ...
        {hud.phase === "brief" && hud.story && (
          ...
          >
            ...
            <p className="brief-route">
              <span className="intel-kicker">
                Road {hud.mapIndex + 1} of {hud.mapTotal}
              </span>
        ...
        {hud.phase === "lost" && (
          <Overlay
            kicker={hud.endless ? "Long Night" : "Breach"}
            title={hud.endless ? "The night took the line" : "The keep fell"}
            emblem
          >
            {hud.endless ? (
              <p className="text-sm text-dust">
                You held{" "}
                {hud.wave > 1
                  ? `${hud.wave - 1} full ${hud.wave - 1 === 1 ? "night" : "nights"}`
                  : "no nights"}{" "}
                on {hud.mapName}. Best night: {hud.bestEndless}.
              </p>
            ) : (
              <p className="text-sm text-dust">
                Wave {hud.wave} of {hud.totalWaves} reached the gate on {hud.mapName}.{" "}
                {hud.lives > 0
                  ? `${hud.lives} ${hud.lives === 1 ? "life" : "lives"} left in the keep.`
                  : "The gate is open."}
              </p>
            )}
            ...
            <div className="defeat-stats" aria-label="Watch state">
              ...
              <span>
                <b>{hud.endless ? hud.bestEndless : `${hud.wave}/${hud.totalWaves}`}</b>{" "}
            ...
            <div className="flex flex-wrap justify-center gap-2">
              <button
                type="button"
                className="pressable min-h-11 bg-copper px-6 text-sm font-semibold text-ink"
          ...
          >
            <div className="selected-tower-copy flex-1">
              <div className="selected-tower-heading">
                ...
                <div
                  className="selected-tower-tags"
                  aria-label={`${hud.formName} form${hud.selectedTower.empowered ? ", Emberlit awakened" : ""}`}
                >
                  <span
                    className="tower-form-chip"
                    data-form={formKey(hud.selectedTower, hud.formName)}
                  >
                    {hud.formName}
                  </span>
                  {hud.selectedTower.empowered && (
                    <span className="tower-ascension-chip">Emberlit</span>
                  )}
                  {hud.kindred && <span className="tower-form-chip">Kindred +10% rate</span>}
                  <span
                    className="tower-form-chip"
                    aria-label={`Power ${hud.selectedTower.dmgLvl}, tempo ${hud.selectedTower.rateLvl}, reach ${hud.selectedTower.rangeLvl}`}
                  >
                    P{hud.selectedTower.dmgLvl} T{hud.selectedTower.rateLvl} R
                    {hud.selectedTower.rangeLvl}
                  </span>
                  {hud.selectedTower.lastUpgrade && (
                    <span
                      className="tower-upgrade-result"
                      data-branch={hud.selectedTower.lastUpgrade}
                      role="status"
                      aria-live="polite"
                    >
                ...
                {hud.moving ? (
                  ...
                  <>
                    ...
                    {playing && hud.phase === "ready" && (
                      <span className="ml-2 text-copper">Next: {hud.nextWave}</span>
                    )}
                  </>
                )}
              </p>
            ...
            <div className="selected-tower-actions grid grid-cols-2 gap-1 sm:grid-cols-6">
              <button
                ...
                disabled={
                  !playing ||
                  hud.selectedTower.dmgLvl >= MAX_UPGRADE ||
                  hud.gold < hud.nextCosts.dmg
                }
              ...
              >
                ...
                {hud.selectedTower.dmgLvl < MAX_UPGRADE && (
                  <span className="upgrade-preview">{upgradePreview(hud, "damage")}</span>
                )}
              </button>
              <button
                type="button"
                ...
                disabled={
                  !playing ||
                  hud.selectedTower.rateLvl >= MAX_UPGRADE ||
                  hud.gold < hud.nextCosts.rate
                }
              ...
              >
                ...
                {hud.selectedTower.rateLvl < MAX_UPGRADE && (
                  <span className="upgrade-preview">{upgradePreview(hud, "rate")}</span>
                )}
              </button>
              <button
                type="button"
                ...
                disabled={
                  !playing ||
                  hud.selectedTower.rangeLvl >= MAX_UPGRADE ||
                  hud.gold < hud.nextCosts.range
                }
              ...
              >
                <span className="upgrade-main">
                  Reach{" "}
                  {hud.selectedTower.rangeLvl >= MAX_UPGRADE ? "max" : `${hud.nextCosts.range}g`}
                </span>
                {hud.selectedTower.rangeLvl < MAX_UPGRADE && (
                  <span className="upgrade-preview">{upgradePreview(hud, "range")}</span>
                )}
              </button>
              <button
                type="button"
                ...
                aria-label={
                  hud.ability
                    ? `${hud.ability.name}: ${hud.ability.detail}${hud.ability.cd > 0 ? `, ready in ${hud.ability.cd} seconds` : ", ready"}`
                    : "No tower selected"
                }
                title={
                  hud.ability
                    ? `${hud.ability.name} — ${hud.ability.detail} (C)`
                    : "No tower selected"
                }
                onClick={() => {
                  unlockAudio();
              ...
              >
                ...
                <span className="upgrade-preview">
                  {hud.ability && hud.ability.cd > 0 ? `${hud.ability.cd}s` : hud.ability?.detail}
                </span>
              </button>
              <button
                type="button"
                className="pressable packet min-h-10 px-3 text-xs text-dust"
            ...
            {hud.emberlitOptions && (
              <div
                className="emberlit-picker"
                role="group"
                aria-label="Choose an Emberlit awakening"
              >
                {(
                  [
                    ["a", hud.emberlitOptions.a],
                    ["b", hud.emberlitOptions.b],
                  ] as Array<[EmberlitBranch, { name: string; detail: string }]>
                ).map(([branch, option]) => (
                  <button
                    key={branch}
                    type="button"
        ...
        <div className="flex flex-wrap items-end gap-2">
              ...
              return (
                ...
                >
                  ...
                  <span className="packet-role">
                    {locked
                      ? "Sealed"
                      : kind === "pike" || kind === "cinder"
                        ? "Low air"
                        : def.hitsAir
                          ? "Air"
                          : "Ground"}
                  </span>
                  <span className="block text-[10px] text-copper">
                    {locked ? "—" : `${def.cost}g`}
                  </span>
                </button>
              );
          ...
          >
            ...
            <button
              ...
              aria-label={
                hud.phase !== "wave"
                  ? "Scout available during a wave"
                  : hud.scoutReady
                    ? "Scout marks the toughest creep"
                    : "Scout already used this wave"
              }
              title={
                hud.phase !== "wave"
                  ? "Scout during a wave"
            ...
            >
              ...
              <span className="command-value">
                {hud.wave < 1 ? "W1+" : hud.phase !== "ready" ? "Between" : "Open"}
              </span>
            </button>
            <button
              type="button"
...
function upgradeAriaLabel(hud: HudSnap, branch: UpgradeBranch, cost: number) {
  ...
  const level =
    branch === "damage" ? tower.dmgLvl : branch === "rate" ? tower.rateLvl : tower.rangeLvl;
  ...
  const current =
    branch === "damage"
      ? towerPower(hud, tower)
      : branch === "rate"
        ? towerRate(hud, tower).toFixed(2)
        : towerReach(hud, tower).toFixed(2);
  const next =
    branch === "damage"
  ...
  const formEffect =
    nextForm !== currentForm ? ` and advances the tower to ${FORM_NAME[nextForm]} form` : "";
...
function RitePicker({ rite }: { rite: WatchRiteId }) {
  const onKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, index: number) => {
    const nextIndex =
      event.key === "ArrowRight" || event.key === "ArrowDown"
        ? (index + 1) % RITES.length
        : event.key === "ArrowLeft" || event.key === "ArrowUp"
          ? (index - 1 + RITES.length) % RITES.length
          : event.key === "Home"
            ? 0
            : event.key === "End"
              ? RITES.length - 1
    ...
    event.preventDefault();
    const next = RITES[nextIndex];
    engine.chooseRite(next.id);
    window.requestAnimationFrame(() => document.getElementById(`rite-${next.id}`)?.focus());
  };
    ...
    <div className="rite-grid" role="radiogroup" aria-label="Watch rite">
      {RITES.map((item, index) => (
        <button
          ...
          role="radio"
          aria-checked={rite === item.id}
          tabIndex={rite === item.id ? 0 : -1}
...
function WatchLedger({ hud }: { hud: HudSnap }) {
  ...
  return (
    <div
      className="watch-ledger"
      aria-label={`${hud.unlocked} of ${hud.mapTotal} routes held and ${hud.relics.length} ${relicLabel} carried`}
    >
    ...
    <div className="briefing-steps" aria-label="First watch steps">
      ...
      <ol>
        <li>
          <b>1</b>
          <span>
            <strong>Choose a packet</strong>
            <small>Bow starts ready for this road.</small>
          </span>
        </li>
        <li>
          <b>2</b>
          <span>
            <strong>Tap open grass</strong>
            <small>Highlighted tiles are safe to plant.</small>
          </span>
        </li>
        <li>
          <b>3</b>
          <span>
            <strong>Send the wave</strong>
            <small>Read the count and hold the keep.</small>
          </span>
        </li>
      </ol>
    </div>
...
function ThreatPanel({
  ...
  hud: HudSnap;
  onSelectCounter: (kind: TowerKind) => void;
}) {
  ...
  return (
    ...
    >
      <div className="intel-heading">
        ...
        <span className={`threat-tier threat-${hud.threatTier}`}>
          {THREAT_LABEL[hud.threatTier]}
        </span>
        <button
          type="button"
      ...
      {!intelOpen && hud.phase === "ready" && hud.lastResult && (
        <CompactWaveRecap result={hud.lastResult} />
      )}
      {!intelOpen && (
          ...
          {hud.wavePreview.map((item) => {
            ...
            return (
              <div
                ...
                aria-label={`${item.count} ${creep.name}${creep.flying ? ", flying" : ""}`}
              >
                <img src={spriteUrl(item.kind)} alt="" />
...
function FieldNote({
  ...
  return (
    <div
      className={`field-note ${compact ? "field-note-compact" : ""}`}
      aria-label={`Field note: ${field.label}`}
    >
      ...
      <div className="field-note-copy">
        ...
        <span className="field-rule-copy">
          {field.rule.label} · {field.rule.detail}
        </span>
      </div>
    </div>
  );
...
function FieldObjective({ objective }: { objective: HudSnap["objective"] }) {
  ...
  return (
    <div
      ...
      aria-label={`Field objective: ${objective.title}`}
    >
      <div className="field-objective-heading">
        ...
        <span className="field-objective-count">
...
function counterPlan(wave: HudSnap["wavePreview"], arsenal: HudSnap["arsenal"] = []): TowerKind[] {
  ...
  const list = COUNTER_ORDER.filter(
    (kind) => needed.has(kind) && (open.size === 0 || open.has(kind)),
  );
...
function WaveRecap({ result }: { result: NonNullable<HudSnap["lastResult"]> }) {
  ...
  return (
    <div
      className="wave-recap"
      data-quality={result.hold}
      aria-label={`Wave ${result.wave} result: ${quality}`}
    >
...
function CompactWaveRecap({ result }: { result: NonNullable<HudSnap["lastResult"]> }) {
  ...
  return (
    ...
    >
      ...
      <strong>{quality}</strong>
      <span>
        {result.kills} cleared · {result.leaks} {breachLabel} · {orderLabel}
      </span>
    </div>
  );
}
    ...
    <section className="tower-intel" aria-label={`${def.name} selected tower details`}>
      <div className="tower-intel-heading">
        ...
        <div className="tower-intel-tags">
          <span className="tower-form" data-form={formKey(tower, hud.formName)}>
            {hud.formName}
          </span>
          {tower.lastUpgrade && (
            <span className="tower-intel-upgrade" data-branch={tower.lastUpgrade}>
              {upgradeResultLabel(tower.lastUpgrade)}
            </span>
          )}
        </div>
      </div>
      ...
      <p className="tower-intel-path">
        Power {tower.dmgLvl} · Tempo {tower.rateLvl} · Reach {tower.rangeLvl} ·{" "}
...
function Overlay({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={title ?? kicker ?? "Emberline"}
    >
      <div
      ...
      >
        {close && onClose && (
          <button
            type="button"
            className="pressable stamp overlay-close min-h-9 px-3 text-[10px] text-dust"
            onClick={onClose}
          >
            {close}
          </button>
        ...
        <img
          className={emblem ? "dispatch-crest" : "wax"}
          src={emblem ? "/ui/emberline-crest-v1.png" : "/ui/wax.png"}
          alt=""
        />
        {kicker && <p className="dispatch-kicker">{kicker}</p>}
        {title && (
          <h2 className="font-display text-4xl leading-none text-copper sm:text-[2.75rem]">
            {title}
          </h2>
        )}
    ...
    <div className="grid w-full grid-cols-2 gap-1.5 text-left md:grid-cols-3">
      {items.map((item) => {
        ...
        const completing = sets.filter(
          (set) =>
            !set.active &&
            set.relics.includes(item.id) &&
            set.relics.every((id) => id === item.id || relics.includes(id)),
        );
        return (
          <button
            key={item.id}
          ...
          >
            <span className="flex items-center justify-between gap-2">
              ...
              <span className="shrink-0 text-xs text-copper">
                {owned ? "Held" : `${item.cost}g`}
              </span>
            </span>
            <span className="mt-0.5 block text-[11px] leading-snug text-dust">{item.blurb}</span>
            {completing.length > 0 && (
              <span className="relic-set-hint">Set · {completing[0].name}</span>
            )}
          </button>
        );
      })}
